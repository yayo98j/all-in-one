...
class DockerActionManager
{
    ...
    public function CreateContainer(Container $container) : void {
        ...
        if ($container->GetIdentifier() === 'nextcloud-aio-nextcloud') {
            $envs[] = $this->GetNextcloudExecCommands();
        }
    ...
    private function GetNextcloudExecCommandsFromDefinition(string $id) : string
    {
        $container = $this->containerDefinitionFetcher->GetContainerById('nextcloud-aio-apache');

        $nextcloudExecCommands = "NEXTCLOUD_EXEC_COMMANDS=";
        foreach ($container->GetDependsOn() as $dependency) {
            $nextcloudExecCommands .= $this->GetNextcloudExecCommands($dependency) . PHP_EOL;
        }
        return $nextcloudExecCommands;
    }

    private function GetNextcloudExecCommands() : string
    {
        $id = 'nextcloud-aio-apache';

        return $this->GetNextcloudExecCommandsFromDefinition($id);
    }
